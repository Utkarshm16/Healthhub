   ...
   try {
    const {email,password}=req.body
     if (email === process.env.ADMIN_EMAIL && password === process.env.ADMIN_PASSWORD) {
        ...
        res.json({success:false,messag:"Invalid Credentials"})
     }
   } catch (error) {
    console.log(error)
    res.json({success:false,message:error.message})
...
const cancelAppointmentAdmin = async(req,res) =>{
    ...
    const { appointmentId } = req.body.headers;
    ...
    res.json({ success: true, message: "Appointment Cancelled",appointments });
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: error.message });
  }
